...
box::use(../global_constant[app_name,app_language, 
                           ...
                           global_seed,api_timeout_seconds,
                           i18n])
...
set_llm_conn <- function(url = "https://openrouter.ai/api/v1/chat/completions",
...
get_json_chat_data <- function(user_input,
                               select_model,
                               history = NULL,
...
get_json_img <- function(user_input,
                         img_url,
                         select_model,
                         image_type = 'file',
                         max_tokens = MAX_TOKENS
...
get_stream_data <- function(req,timeout_seconds = api_timeout_seconds){
  ...
  if ('try-error' %in% class(response) ) {
    ...
    if (resp_is_error(response)) {
      ...
      response_message <- list(model = NULL, choices = list(list(
        message = list(role = 'error', content = error_code),
...
llm_chat <- function(user_input, model_id='llama', history=NULL){
  ...
  response_message <- get_llm_result(prompt, model_id, history = chat_history, llm_type =
                                       'chat') 
